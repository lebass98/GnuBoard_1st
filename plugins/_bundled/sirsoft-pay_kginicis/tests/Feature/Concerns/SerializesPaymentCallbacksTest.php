...
namespace Plugins\Sirsoft\PayKginicis\Tests\Feature\Concerns;
...
class SerializesPaymentCallbacksTest extends PluginTestCase
{
    ...
    {
        $orderNumber = 'ORD-KG-LOCK-' . random_int(10000, 99999);
    ...
    {
        $this->assertNotSame(
            $this->traitSubject->callbackLockKey('authCallback', 'ORD-KG-LOCK-A'),
            $this->traitSubject->callbackLockKey('vbankNotify', 'ORD-KG-LOCK-A'),
        );

        $this->assertNotSame(
            $this->traitSubject->callbackLockKey('authCallback', 'ORD-KG-LOCK-A'),
            $this->traitSubject->callbackLockKey('authCallback', 'ORD-KG-LOCK-B'),
        );
    }
}
